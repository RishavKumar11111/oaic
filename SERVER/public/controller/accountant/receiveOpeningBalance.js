...
app.controller("myCtrl", async function($scope, $http) {
    ...
    $scope.c_date = new Date(cDate);
    $scope.print_page = false;
    $scope.enter_detail1 = true;
    $scope.enter_detail2 = true;
    $scope.system = 'farm_mechanisation';
    $scope.type = 'new_order';
    ...
    $scope.receiveOPBalance = () => {
        ...
        $scope.enter_detail1 = false;
        $scope.enter_detail2 = false;
    }
    $scope.back = () => {
        $scope.print_page = false;
        $scope.enter_detail1 = true;
    ...
    $scope.submit = async() => {
        let data = {
            ...
            payment_no: $scope.payment_no,
            remark: $scope.remark,
            dist_id: dist_id,
            from: $scope.receive_from
        }
        if ($scope.type == 'old_order') {
            data.order_no = $scope.old_order_no.order_no;
        } else {
            data.order_no = $scope.order_no;
        }
        try {
            var req = {
                method: 'POST',
                url: '/accountant/addReceivedOpeningBalance',
                headers: {
                    'csrf-token': token
                },
                data: data
