...
app.controller("myCtrl", function($scope, $http) {
    ...
    $scope.loadInvoices = function() {
        ...
        $scope.show_invoice_list = false;
        $scope.loader = true;
        $http.get('/dl/getFinYearWisePendingApprovalList?fin_year=' + $scope.fin_year + '&dl_id=' + dl_id)
            .then(response => {
    ...
    $scope.loadDetail = (x) => {
        ...
        $scope.paid_amount = x.ammount - x.deduction_amount;
        $scope.pending_amount = x.deduction_amount;
        $scope.remark = x.remark;

    }
    $scope.resolveIssue = async() => {
        try {
            var req = {
                method: 'POST',
                url: '/dl/updateApprovalToPayble',
                headers: {
                    'csrf-token': token
                },
                data: { approval_id: $scope.approval_id, dl_remark: $scope.dl_remark }
            }
            await $http(req);
            window.alert('Thank you. The withheld amount resolved successfully.');
            window.location.href = '/dl/withheldList';
        } catch (e) {
            window.alert('Failed to resolve that withheld amount. Please try again');
