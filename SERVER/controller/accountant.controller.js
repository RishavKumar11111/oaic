...
exports.addIndent = async (req, res, next) => {
    try {
        ...
        const DistID = req.payload.DistrictID;
        const DistCode = await db.sequelize.query(`select "DistCode" from "DistrictMaster" where dist_id = '${DistID}'`);
        const DistrictFullName1 = await db.sequelize.query(`select dist_name from "DistrictMaster" where dist_id = '${DistID}'`);
        const DistrictFullName = DistrictFullName1[0][0].dist_name;
        const DistrictName =DistCode[0][0].DistCode;
        const fin_year = getCurrentFinYear();
        const max = await db.sequelize.query(`SELECT max(cast(substr("PONo", 16, length("PONo")) as int )) from "POMaster" where "FinYear"='${fin_year}' and "DistrictID"='${DistID}'`);        
        const PONumber = max[0][0].max == null ? `${DistrictName}/PO/${fin_year}/1` : `${DistrictName}/PO/${fin_year}/${(parseInt(max[0][0].max) + 1)}`;
        ...
        orderList.forEach((e, i) => {
            ...
            e.DistrictName = DistrictFullName;
...
exports.addMRR = async (req, res, next) => {
    ...
    try {
        ...
        const DistrictID = req.payload.DistrictID;
        const DistCode = await db.sequelize.query(`select "DistCode" from "DistrictMaster" where dist_id = '${DistrictID}'`);
        const DistrictName =DistCode[0][0].DistCode;
        ...
        let fin_year = getCurrentFinYear();
        let mrr_id = max.rows[0].max == null ? `${DistrictName}/${fin_year}/1` : `${DistrictName}/${fin_year}/${(parseInt(max.rows[0].max) + 1)}`;
...
exports.getFinYearWiseInvoiceList = async (req, res, next) => {
    try {
        const fin_year = req.query.fin_year;
        const DistrictID = req.payload.DistrictID
        let queryText = `SELECT b."LegalBussinessName", b."TradeName", a.*, c."POAmount", c."ApprovedDate" FROM "InvoiceMaster" a
        INNER JOIN "VendorMaster" b ON b."VendorID" = a."VendorID"
        INNER JOIN "POMaster" c ON a."PONo" = c."PONo"
        WHERE a."FinYear"='${fin_year}' AND a."DistrictID" = '${DistrictID}'`;
        let response = await db.sequelize.query(queryText);
        res.send(response[0]);
    } catch (e) {
        console.error(e);
...
exports.getInvoiceItemsForPay = async (req, res, next) => {
    try {
        ...
        LEFT JOIN "ItemMaster" d ON c.implement = d."Implement" AND c.make = d."Make" AND c.model = d."Model"
        LEFT JOIN "POMaster" e ON e."PONo" = a."PONo"
...
exports.addPaymentApproval = async ({ approval, apDesc, updateInvoice }) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // console.log("APDesc",apDesc);
            // console.log("UPADATE",updateInvoice);
...
exports.getDistrictWiseVendorList = async (req, res, next) => {
    try {
        ...
        INNER JOIN "VendorMaster" b ON a."VendorID" = b."VendorID"
        WHERE a."DistrictID" = '${districtID}' and b."ApprovalStatus" = 'Approved'`;
        const response = await db.sequelize.query(queryText);
        res.send(response[0]);
    } catch (e) {
        console.error(e);
...
exports.addCustomerInvoice = async (req, res, next) => {
    try {
        ...
        const DistrictName =DistCode[0][0].DistCode;
        const fin_year = getCurrentFinYear();
        const max = await db.sequelize.query(`SELECT max(cast(substr("CustomerInvoiceNo", 16, length("CustomerInvoiceNo")) as int )) from "CustomerInvoiceMaster" where "FinYear"='${fin_year}' and "DistrictID" = '${DistrictID}'`);
        const CustomerInvoiceNo = max[0][0].max == null ? `${DistrictName}/IN/${fin_year}/1` : `${DistrictName}/IN/${fin_year}/${(parseInt(max[0][0].max) + 1)}`;
